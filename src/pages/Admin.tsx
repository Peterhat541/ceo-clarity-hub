...
export default function Admin() {
  ...
  // Derive unique project managers from clients
  const projectManagers = useMemo(() => {
    const managers = clients
      .map(c => c.project_manager)
  ...
  const filteredAndSortedClients = useMemo(() => {
    let result = clients.filter((client) => {
      ...
      const matchesEmployee = !selectedEmployee || selectedEmployee === "all" || client.project_manager === selectedEmployee;
    ...
    return result;
  }, [clients, searchQuery, statusFilter, sortField, sortDirection, selectedEmployee]);
  ...
  const handleSaveNote = async (data: {
    ...
    try {
      // Find client_id if a client name is provided
      ...
      const createdBy = (selectedEmployee && selectedEmployee !== "all") ? selectedEmployee : "Admin";

      const { error } = await supabase.from("notes").insert({
        text: `${data.title}: ${data.content}`,
        visible_to: data.visibility,
        client_id: clientId,
        created_by: createdBy,
        due_at: data.targetDate.toISOString(),
        status: "pending",
      });
    ...
    <div className="flex flex-col min-h-screen bg-background bg-grid">
      ...
      {!isMobile && (
        <div className="h-14 border-b border-border bg-background flex items-center justify-end px-6 gap-3">
          ...
          {(statusFilter !== "all" || searchQuery || (selectedEmployee && selectedEmployee !== "all")) && (
            <Button 
              variant="ghost" 
              onClick={() => { setStatusFilter("all"); setSearchQuery(""); setSelectedEmployee("all"); }}
          ...
          <div className="flex items-center gap-2">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent className="bg-popover border-border">
                ...
                <DropdownMenuItem onClick={() => setStatusFilter("red")}>
                  <span className="w-2 h-2 rounded-full bg-status-red mr-2" /> Rojos
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => setStatusFilter("orange")}>
                  <span className="w-2 h-2 rounded-full bg-status-orange mr-2" /> Naranjas
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => setStatusFilter("yellow")}>
                  <span className="w-2 h-2 rounded-full bg-status-yellow mr-2" /> Amarillos
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => setStatusFilter("green")}>
                  <span className="w-2 h-2 rounded-full bg-status-green mr-2" /> Verdes
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
      ...
      {isMobile && (
        <main className="flex-1 overflow-auto p-4 space-y-3">
          <div className="flex items-center justify-between mb-2">
            <p className="text-sm text-muted-foreground">{filteredAndSortedClients.length} clientes</p>
            <button 
              onClick={() => {
                setSelectedClient(null);
                openModal("newNote");
              }}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-status-purple/10 text-status-purple text-xs font-medium"
            >
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {client.project_manager && (
                    <p className="text-xs text-muted-foreground">Resp: {client.project_manager}</p>
                  )}
                  {client.incidents && (
                    <p className="text-xs text-status-orange mt-1 flex items-center gap-1">
                      <AlertTriangle className="w-3 h-3" />
                      {client.incidents.length > 40 ? client.incidents.slice(0, 40) + "..." : client.incidents}
                    </p>
                  )}
                </div>
              </div>
